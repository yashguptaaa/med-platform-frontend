...
export default function EditHospitalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [specsRes, hospitalRes] = await Promise.all([
        ...
        const hospital = hospitalRes.data.data;
        setFormData({
          name: hospital.name,
          city: hospital.city,
          ...
          specializationIds: hospital.specializations.map((s: Specialization) => s.id),
        });
      } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-800 mb-6">Edit Hospital</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">
            Hospital Images (Optional, max 5)
          </label>
          <input
            type="file"
            accept="image/*"
          ...
          />
          <p className="text-xs text-slate-500 mt-1">Uploading new images will replace all existing images.</p>
        </div>
